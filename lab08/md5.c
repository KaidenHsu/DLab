...
void md5(uint8_t *initial_msg, size_t initial_len, uint8_t *hash)
{
    ...
    uint8_t *p; // a pointer to store output hash
    ...
    // Print the msg buffer after preprocessing
    printf("msg buffer after preprocessing (64 bytes):\n");
    for (int i = 0; i < 64; i++) {
        printf("%02x ", msg[i]);
        if ((i + 1) % 16 == 0) printf("\n");
    }
    ...
    for (int offset = 0; offset < pad_len; offset += (512/8))
    {
        ...
        for(uint32_t i = 0; i < 64; i++)
        {
            ...
            printf("i=%2d: a=%08x b=%08x c=%08x d=%08x f=%08x g=%2d\n", i, a, b, c, d, f, g);
        }
